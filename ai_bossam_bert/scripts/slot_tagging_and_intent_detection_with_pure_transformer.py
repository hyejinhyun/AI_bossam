...
parser.add_argument('--dataset', required=True, help='atis-2')
...
parser.add_argument('--pretrained_model_type', required=True, help='bert')
...
# --pretrained_model_type=bert --pretrained_model_name=bert-base-uncased
...
def create_file_train(slot, intent):
    f = open('output_bert_train.csv', 'a', encoding='utf-8')
    wr = csv.writer(f)
    wr.writerow([slot, intent])
    f.close()


def create_file_val(slot, intent):
    f = open('output_bert_val.csv', 'a', encoding='utf-8')
    wr = csv.writer(f)
    wr.writerow([slot, intent])
    f.close()


def create_file_eval(slot, intent):
    f = open('output_bert_eval.csv', 'a', encoding='utf-8')
    wr = csv.writer(f)
    wr.writerow([slot, intent])
    f.close()
...
if not opt.testing:
    ...
    for i in range(opt.max_epoch):
        ...
        create_file_train(mean_loss[0], mean_loss[1])
        ...
        create_file_val(loss_val[0], loss_val[1])
        ...
        create_file_eval(loss_te[0], loss_te[1])
