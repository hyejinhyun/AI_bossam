...
#from models.fast_elmo import FastElmo as Elmoword_lowercase
...
def create_file_train(slot, intent):
    f = open('output_elmo_train.csv', 'a', encoding='utf-8')
    wr = csv.writer(f)
    wr.writerow([slot, intent])
    f.close()
    
def create_file_val(slot, intent):
    f = open('output_elmo_val.csv', 'a', encoding='utf-8')
    wr = csv.writer(f)
    wr.writerow([slot, intent])
    f.close()
    
def create_file_eval(slot, intent):
    f = open('output_elmo_eval.csv', 'a', encoding='utf-8')
    wr = csv.writer(f)
    wr.writerow([slot, intent])
    f.close()
...
if not opt.testing:
    ...
    for i in range(opt.max_epoch):
        ...
        logger.info('Training:\tEpoch : %d\tTime : %.4fs\tLoss of slot : %.2f\tLoss of intent : %.2f ' % (i, time.time() - start_time, mean_loss[0], mean_loss[1]))
        gc.collect()
